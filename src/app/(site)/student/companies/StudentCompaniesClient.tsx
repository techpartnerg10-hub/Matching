...
import { searchCompanies, getKeywordName } from "@/lib/demoActions";
...
export default function StudentCompaniesClient() {
  ...
  return (
    <RequireRole roles={["student"]} redirectTo="/">
      <ClientOnly fallback={<div className="text-sm text-[color:var(--muted)]">로딩 중…</div>}>
        <StudentCompaniesInner keywordIds={keywordIds} mode={mode} />
      </ClientOnly>
    </RequireRole>
  );
}

function StudentCompaniesInner({ keywordIds, mode }: { keywordIds: string[]; mode: "all" | "any" }) {
  const router = useRouter();
  const db = loadDb();
  const { results } = searchCompanies({ keywordIds, mode });
  ...
  function onApplyFilter() {
    ...
    router.push(`/student/companies?${qs.toString()}`);
